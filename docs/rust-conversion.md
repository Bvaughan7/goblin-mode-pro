...
| **P4** | `gmp-daemon` and `gmp-cli` | **In progress.** Five slices in: the `selftest` reporting layer, launch resolution, the observer's process matching, the cold-revert state reader, and what the CLI prints. The probing, the poll loop and everything holding a file descriptor are still Python. Two of the five found a crash |
