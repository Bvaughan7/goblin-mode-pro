...
| **P4** | `gmp-daemon` and `gmp-cli` | **In progress.** Seven slices in: the `selftest` reporting layer, launch resolution, the observer's process matching, the cold-revert state reader, what the CLI prints, the bug-report renderer, and the daemon's read-and-report answers. What is left in `daemon.py` is the GLib loop and the state it owns, which is a rewrite against the frozen interface rather than an extract-and-diff |
