...
| **P4** | `gmp-daemon` and `gmp-cli` | **In progress.** Seven judgement slices are ported, and `crates/gmp-daemon` now serves the frozen interface as refusing stubs - graded byte for byte by the same canonicalizer the Python daemon goes through. What is left is the GLib loop and the state it owns, which is a rewrite against that contract rather than an extract-and-diff |
