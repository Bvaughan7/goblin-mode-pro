...
| **P4** | `gmp-daemon` and `gmp-cli` | **In progress.** Six slices in: the `selftest` reporting layer, launch resolution, the observer's process matching, the cold-revert state reader, what the CLI prints, and the bug-report renderer. The probing, the poll loop and everything holding a file descriptor are still Python. Three of the six found a defect |
